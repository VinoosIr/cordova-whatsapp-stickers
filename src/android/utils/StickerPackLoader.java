...
import static miladesign.cordova.wastickers.utils.StickerContentProvider.LICENSE_AGREENMENT_WEBSITE;
...
class StickerPackLoader {
    ...
    static Uri getStickerAssetUri(String identifier, String stickerName) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(StickerContentProvider.CONTENT_PROVIDER_AUTHORITY).appendPath(StickerContentProvider.STICKERS_ASSET).appendPath(identifier).appendPath(stickerName).build();
    }
}